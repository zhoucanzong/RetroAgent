...
class EvaluationTool:
    ...
    def execute(self, parameters: dict) -> str:
        ...
        for rxn in reactions:
            ...
            feasibility = self._predict_feasibility(rxn.get("product_smiles", ""),
                                                     rxn.get("precursor_smiles_list", []))
            stock_avail = self._check_stock_availability(rxn.get("precursor_smiles_list", []))
            scores[rid] = {
                "feasibility": round(feasibility, 4),
                "stock_availability": round(stock_avail, 4),
                "precursor_count": len(rxn.get("precursor_smiles_list", [])),
    ...
    def _check_stock_availability(self, precursor_smiles_list: list[str]) -> float:
        if not self._stock_keys or not precursor_smiles_list:
            return 0.5
        in_stock = 0
        for smi in precursor_smiles_list:
            try:
